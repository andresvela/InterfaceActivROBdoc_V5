...
public class ToBdocWeb extends Action {
	...
	public WebServiceRetour execute(byte[] xmlflow) throws BdocWebServiceInterfaceException{
		...
		try {
			...
			if (templateFindCriteriaIndex==null)
			{
				document = bdocCompose.BdocComposeDocument(domainValue, templateValue, xmlflow,pdfSecurityOption);
			}
			else
			{
				document = bdocCompose.BdocComposeDocument(templateFindCriteria, xmlflow,pdfSecurityOption);				
			}
			...
			BdocWebServiceInterfaceException ex = new BdocWebServiceInterfaceException(102);
